    ...
    <header
      className={
        theme === "light"
          ...
          : "bg-slate-800 text-white px-6 shadow"
      }
    >
      <div className="flex h-16 max-w-6xl mx-auto items-center justify-between">
        <button // Boton del icono del menu
          className="md:hidden -ml-1 rounded p-1 text-slate-500 transition-colors hover:bg-sky-500
        ...
        <div className="mx-auto flex items-center">
          {/* Contiene el icono del sombrerito y la vista de la navegacion de escritorio*/}
          <Link className="duration-200 hover:rotate-6" href="#">
            <svg
              className="h-8 w-8"
              role="img"
              viewBox="0 0 24 24"
          ...
          <div className="ml-8 hidden md:flex space-x-8">
            {/* Contiene la vista de la navegación de escritorio */}
            {links.map((link) => (
              <NavDesktop key={link.href} href={link.href}>
                {link.name}
              </NavDesktop>
            ))}
          </div>
        ...
        <div className="flex">
          {/* Contiene el boton con el icono del sol-luna y el boton con el icono del usuario */}
          <button
            className="rounded-full text-slate-500 transition-colors hover:text-sky-500 
          ...
          <button
            className="ml-4 rounded-full text-slate-500 transition-colors hover:text-sky-500 
          focus:ring-2 focus:ring-slate-200 focus:ring-offset-1 "
          >
            <img
              className="h-6 w-6 rounded-full"
              src="https://ui-avatars.com/api?name=Nelson+Cruz"
              alt="NC"
            />
          </button>
        </div>
        ...
        {isMenuOpen && (
          <div className="space-y-1 border-t pb-3 pt-2 md:hidden">
            {/* Contiene la vista de la navegación móvil */}
            {links.map((link) => (
              <NavMovil key={link.href} href={link.href}>
                {link.name}
              </NavMovil>
            ))}
          </div>
